...
export default async function JournalPage() {
  // 1. Tarik semua data mentah dari Google Sheet
  const rawTrades = (await fetchSheetData()) || [];

  // 2. Hitung statistik langsung dari semua data
  const stats = calculateStats(rawTrades);
      ...
      <PageTransition>
        <div className="max-w-5xl mx-auto px-4 md:px-6 space-y-10 relative z-10">
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl hover:border-white/20 transition-all">
            <EquityChart trades={rawTrades} />
          </div>

          {/* Tabel Jurnal Utama (Tampil Semua) */}
