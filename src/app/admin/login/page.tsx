...
import { supabase } from "@/lib/supabase"; // Kabel database kita
import {
  Lock,
  Mail,
  KeyRound,
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Mantra untuk ngecek ke Supabase apakah Email & Password cocok
    ...
    if (error) {
      ...
      // Kalau cocok, langsung tendang ke halaman Dashboard Admin
      router.push("/admin");
    }
  };
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4 text-slate-200 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-rose-600/10 rounded-full blur-[150px] -z-10 pointer-events-none" />
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden">
            ...
            <div className="text-center mb-10 relative z-10">
              ...
              <p className="text-slate-400 text-sm">
                Silakan login untuk mengelola konten web
              </p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-6 relative z-10">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-900 font-bold py-4 rounded-xl transition-all shadow-[0_0_20px_rgba(245,158,11,0.2)] hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:hover:scale-100 mt-4"
              >
                {loading ? (
